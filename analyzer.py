...
def save_to_html(data, filename="../reports/phishing_report.html"):
    ...
    risk_class = data.get("risk_rating", "").lower().split()[0]
    ...
    <body>
        ...
        <div class="risk {risk_class}">
            Risk Rating: {data.get("risk_rating")} |
        ...
        <table>
            ...
            <tr>
                <td>Risk Score</td>
                <td>{data.get("risk_score")}</td>
            </tr>

            <tr>
                <td>Risk Rating</td>
                <td>{data.get("risk_rating")}</td>
            </tr>
        </table>
        ...
        <table>
            ...
            <tr>
                <td>Email Domains</td>
                <td>{"<br>".join(data.get("email_domains", []))}</td>
            </tr>

            <tr>
                <td>Suspicious TLDs</td>
                <td>{"<br>".join(data.get("suspicious_tlds", []))}</td>
            </tr>

            <tr>
                <td>IP Addresses</td>
                <td>{"<br>".join(data.get("ip_addresses", []))}</td>
            </tr>

            <tr>
                <td>URLs</td>
                <td>{"<br>".join(data.get("urls", []))}</td>
            </tr>

            <tr>
                <td>Suspicious Keywords</td>
                <td>{"<br>".join(data.get("keywords", []))}</td>
            </tr>

            <tr>
                <td>Domain Findings</td>
                <td>{"<br>".join(data.get("domain_findings", []))}</td>
            </tr>
...
def analyze_phishing_email(message_id: str):
    # Your existing phishing logic goes here
    ...
    return {
        ...
        "findings": [
            "Suspicious sender",
            "Possible phishing link detected"
        ],
...
def analyze_phishing_email(email_data):
    ...
    if suspicious_tlds:
        findings.append(
            f"Suspicious TLDs detected: {', '.join(suspicious_tlds)}"
        )
    ...
    for attachment in attachments:
        ...
        if filename.endswith(
            ...
            risk_score += 80
            findings.append(
                f"High-risk attachment detected: {filename}"
            )
            ...
            risk_score += 60
            findings.append(
                f"Macro-enabled attachment detected: {filename}"
            )
            ...
            risk_score += 40
            findings.append(
                f"Compressed archive attachment detected: {filename}"
            )
    ...
    if risk_score >= 80:
        ...
        recommendation = (
            ...
            "Report this email to IT."
        )

    elif risk_score >= 40:
        risk_level = "Suspicious"
        recommendation = (
            "Use caution. Do not open attachments until reviewed."
        )
